...
export default function search<T extends Record<string, any>>(items: T[], keys: string[], searchString: string) {
  ...
  const results = [...allResults, ...wordResults].filter((item, index, self) => index === self.findIndex((t) => (t as any).title === (item as any).title));
